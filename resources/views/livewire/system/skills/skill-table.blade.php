            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($skills as $skill)
                    <tr class="cursor-pointer hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">{{ $skill->skill_title }}</td>
                        ...
                        <td class="w-9 text-center">
                            <div class="ms-3 mx-auto">
                                <x-dropdown align="center" width="48">
                                    ...
                                    <x-slot name="content">
                                        <!-- Edit Button -->
                                        <x-dropdown-link href="{{ route('skills.edit', $skill->id) }}">
                                            Edit
                                        </x-dropdown-link>

                                        <!-- Delete Form -->
<form id="delete-form-{{ $skill->id }}" action="{{ route('skills.destroy', $skill->id) }}" method="POST">
    @csrf
    @method('DELETE')
    <x-dropdown-link>
        <button type="button" onclick="confirmDelete({{ $skill->id }})" class="text-red-900 hover:bg-red-200/50">
            Delete
        </button>
    </x-dropdown-link>
</form>
...
<script>
    function confirmDelete(skillId) {
        if (confirm("Are you sure you want to delete this skill?")) {
            ...
            // Do nothing or redirect if needed
        }
    }
</script>
