<x-form-section submit="createJCPInformation">
    ...
    <x-slot name="form">
        ...
        <div class="col-span-6 sm:col-span-4">
            ...
            <textarea id="job_purpose" wire:model.defer="job_purpose" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-fuchsia-500 focus:border-fuchsia-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-fuchsia-500 dark:focus:border-fuchsia-500" placeholder="Write your thoughts here..."></textarea>
            <x-input-error for="job_purpose" class="mt-2" />
        </div>

        <div class="col-span-6 sm:col-span-4">
            <label class="inline-flex items-center cursor-pointer">
                <input type="checkbox" wire:model.defer="is_active" value="" class="sr-only peer">
        ...
        <div class="col-span-full sm:col-span-4">
            ...
            <select id="jcp_qualifications.qualification_id" wire:model.defer="jcp_qualifications.qualification_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            ...
            <x-input-error for="jcp_qualifications.qualification_id" class="mt-2" />
        </div>
        ...
        <div class="col-span-6 sm:col-span-4">
            ...
            @forelse ($prerequisites as $prerequisite)
                <div class="flex items-center ps-4 border border-gray-200 rounded dark:border-gray-700 mb-2">
                    <input id="jcp_prerequisites" type="radio" value="{{ $prerequisite->id }}" wire:model.defer="jcp_prerequisites" name="jcp_prerequisites" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                    <label for="jcp_prerequisites" class="w-full py-4 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $prerequisite->prerequisite_title }}</label>
                </div>
            @empty
                <div class="flex items-center ps-4 border border-gray-200 rounded dark:border-gray-700 mb-2">
                    <input checked id="jcp_prerequisites" type="radio" value="" name="jcp_prerequisites" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                    <label for="jcp_prerequisites" class="w-full py-4 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">No Prerequisites Loaded</label>
                </div>
            @endforelse
            ...
            <table>
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <select name="skills" wire:model.defer="jcp_skills.{{ $index }}.skill_id" class="rounded-lg">
                            @foreach ($skills as $availableSkill)
                                <option value="{{ $availableSkill->id }}">
                                    {{ $availableSkill->skill_title }}
                                </option>
                            @endforeach
                        </select>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <select name="jcp_skills.{{ $index }}.required_rating" wire:model.defer="jcp_skills.{{ $index }}.required_rating" class="rounded-lg">
    ...
    <x-slot name="actions">
        @if($currentPage === 1)
            <x-secondary-button wire:click.prevent='previousPage' class="ms-2" disabled>
                {{ __('Previous') }}
            </x-secondary-button>
        @elseif($currentPage > 1)
            <x-secondary-button wire:click.prevent='previousPage' class="ms-2">
                {{ __('Previous') }}
            </x-secondary-button>
        @endif
